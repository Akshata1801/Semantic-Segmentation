...
class segnet_skip(nn.Module):
    def __init__(self):
        ...
        self.conv60 = nn.Conv2d(512, 512, kernel_size=3,stride=1, padding=1)
        self.batch_norm60 = nn.BatchNorm2d(512, momentum=batchNorm_momentum,affine=False)
        
        
        
        self.conv72 = nn.Conv2d(1024,512, kernel_size=3,stride=1,padding=1)
        ...
        self.dec_downsample5=nn.Conv2d(256,256,kernel_size=3,stride=1,padding=(5,9))
        self.dec_downsample6=nn.Conv2d(256,256,kernel_size=3,stride=1,padding=(5,9))
        self.dec_batch2=nn.BatchNorm2d(256, affine=False,momentum=batchNorm_momentum)
        
        self.conv82 = nn.Conv2d(512,256,kernel_size=3,stride=1,padding=1)
        ...
        self.conv91 = nn.Conv2d(256,128,kernel_size=3,stride=1, padding=1)
        ...
        self.conv101 = nn.Conv2d(128,64, kernel_size=3,stride=1,padding=1)
...
#        print("x4_unpool.size()", x4_unpool.size())
#        print("x4_pool size()", x4_pool.size())
#        print("x4_pool1 size",x4_pool1.size())
        x42_dec = F.relu(self.batch_norm72(self.conv72(torch.cat((x4_unpool,x4_pool1),1))))
...
#        print("x3_unpool.size()", x3_unpool.size())
#        print("x3_pool1 size()", x3_pool1.size())
        #print("x31_unpool size",x41_unpool.size())
        
        x32_dec = F.relu(self.batch_norm82(self.conv82(torch.cat((x3_unpool,x3_pool1),1))))
...
#        print("x2_unpool.size()", x2_unpool.size())
#        print("x2_pool1 size()", x2_pool1.size())
        
        x21_dec = F.relu(self.batch_norm91(self.conv91(torch.cat((x2_unpool,x2_pool1),1))))
...
#        print("x1_pool1 size()", x1_pool1.shape)
        x11_dec = F.relu(self.batch_norm101(self.conv101(torch.cat((x1_unpool,x1_pool1),1))))
